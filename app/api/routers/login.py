...
from orm import UserModel, SupplierModel, CompanyModel
...
async def current(
    ...
    if user.supplier:
        res = await crud.companies.select.one(
            Where(CompanyModel.supplier_id == user.supplier.id), session=session
        )
        # breakpoint()
        return {
            "ok": True,
